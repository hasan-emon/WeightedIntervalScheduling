...
public class WISMemoization {
	
	private List<Job> jobs;
	private int[] M;
	private ArrayList<Integer> includedJobs = new ArrayList<Integer>();

	public void calcSchedule(List<Job> inputJobs){
		jobs= inputJobs;
		M = new int[jobs.size()];	//create memoization array

		Collections.sort(jobs, new Comparator<Job>() {
			@Override
			public int compare(Job j1, Job j2) {
				return Integer.compare(j1.getValue(), j1.getValue());
			}
		});	//Sort jobs by finish time
		
		
		M[0]=0;		//base case with no jobs selected
		
		for(int i = 1; i<jobs.size(); i++){
			M[i] = Math.max( jobs.get(i).getValue()+M[latestCompatible(i)],   M[i-1] );		//add max value if job is included or if it's not included
		}

//		findSolutionIterative(memo.length-1);		//Recursively find solution & update includedJobs
		WISTopDown(M.length-1);
...
//		System.out.println("Memoization array: " + Arrays.toString(M));
		System.out.println("Total weight: " + M[M.length-1]);
	}
	...
	//Find the index of the job finishing before job i starts (uses jobs[][] array sorted by finish time)
		private int latestCompatible(int i){
			...
			while (low <= high){		//Iterative binary search
				...
				if (jobs.get(mid).getFinishTime() <= jobs.get(i).getStartTime()) {
					if (jobs.get(mid+1).getFinishTime() <= jobs.get(i).getStartTime())
						low = mid + 1;
					else
						return mid;
				}
		...
		public void WISBottomUp(int j){
			while (j>0){	//Stops when j==0
				int compatibleIndex = latestCompatible(j);	//find latest finishing job that's compatible with job j
				if(jobs.get(j).getValue()+ M[compatibleIndex] > M[j-1]){	//Case where job j was included (from optimal substructure)
		...
		private void WISTopDown(int j){
			if(j==0){	//base case
				...
				if(jobs.get(j).getValue()+ M[compatibleIndex] > M[j-1]){	//Case where job j was included (from optimal substructure)
					includedJobs.add(j);	//add job index to solution
					WISTopDown(compatibleIndex);	//recursively find remaining jobs starting the the latest compatible job
				}
		...
		private String getJobInfo(int jobIndex){
			return jobs.get(jobIndex).getId() + " " + jobs.get(jobIndex).getStartTime() +" " + jobs.get(jobIndex).getFinishTime() +" " + jobs.get(jobIndex).getValue();
		}
